...
<td><input type="submit" name="search" value="Search Teacher ID" id="search" class="btn btn-md btn-primary w-100 fs-6"></tr></table></form>';

if(isset($_POST['search'])){
	$search = mysqli_real_escape_string($link, $_POST['searchquery']);
	$query = mysqli_query($link, "SELECT * FROM teacher WHERE TEACHER_ID LIKE '%$search%'");
	
	if(mysqli_num_rows($query)!=0){
		echo '<table border="1" id="record"  class="table table-dark"><tr>
			<th>TEACHER_ID</th> <th>PASSWORD</th> <th>EMAIL_ADDRESS</th> <th>NAME</th> <th>GENDER</th> 
			<th>PHONE</th> <th>DEPARTMENT</th> <th>EDUCATIONAL_LEVEL</th><th></th><th></th>
		</tr>';
		while($row = mysqli_fetch_array($query)){
			echo '<tr>
				...
				<td><input type="button" onclick="window.location = \'teacherEdit.php?TEACHER_ID='.$row['TEACHER_ID'].'\'" 
				value="Edit" class="btn btn-md btn-success w-100 fs-6"></td>
				<td><input type="button" onclick="window.location = \'teacherDelete.php?TEACHER_ID='.$row['TEACHER_ID'].'\'" 
