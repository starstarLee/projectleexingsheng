...
<td><input type="submit" name="search" value="Search Subject ID"id="search" class="btn btn-md btn-primary w-100 fs-6"></tr></table></form>';

if(isset($_POST['search'])){
	$search = mysqli_real_escape_string($link, $_POST['searchquery']);
	$query = mysqli_query($link, "SELECT * FROM subject WHERE SUBJECT_ID LIKE '%$search%'");
	
	if(mysqli_num_rows($query)!=0){
		echo '<table border="1" id="record" class="table table-dark"><tr>
			<th>SUBJECT_ID</th> <th>SUBJECT_NAME</th> <th></th><th></th>
		</tr>';
		...
		<td><input type="button" onclick="window.location = \'subjectEdit.php?SUBJECT_ID='.$row['SUBJECT_ID'].'\'" value="Edit" class="btn btn-md btn-success w-100 fs-6"></td>
		<td><input type="button" onclick="window.location = \'subjectDelete.php?SUBJECT_ID='.$row['SUBJECT_ID'].'\'" value="Delete" class="btn btn-md btn-danger w-100 fs-6">
		</td>
		</tr>';
		}
		echo '</table>';
...
$query = mysqli_query($link, "SELECT * FROM subject");
